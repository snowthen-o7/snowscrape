...
**Build Status:** PASSES (backend full suite 433 passed green as of 2026-06-09; frontend lint 0 errors + vitest 118/118 + prod build green on claude-main as of 2026-06-08)
...
- JSON (live), CSV, XLSX, SQL export (rewritten pandas-free 2026-06-09, #32; reach prod on the next backend deploy)
- Parquet: gated. Needs pyarrow, which is not bundled in the Lambda (size), so a Parquet request returns a clean error. Decide later: bundle pyarrow vs drop the format from the UI.
